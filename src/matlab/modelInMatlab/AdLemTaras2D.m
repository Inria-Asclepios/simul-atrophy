% Faster method for creating the sparse matrix:
...
% 
function [v2 p] = AdLemTaras2D(seg_mask,div_slice,USE_REAL_DATA,...
...
%     brain_mask_test = cat(2,zeros(m, min_max_row_col(3)-extra_CSF-1-1),brain_mask_test,zeros(m, n-min_max_row_col(4)-extra_CSF-1+1));
    ...
    brain_mask = circleMask(center_mask,xnum/3,xnum-1,ynum-1);
    % brain_mask = circleMask(center_mask,7,xnum-1,ynum-1);

    % atrophy at the center of the cells
    a = computeAtrophy(ynum-1,xnum-1);
    a(brain_mask==false) = 0;
    % distributed source outside brain, excluding 4 corners where CC is not
    % enforced!
    a(brain_mask==false) = -sum(a(:))/(xnum*ynum - 4 - sum(brain_mask(:)));
     % add a dummy atrophy corresponding to dummy pressure values!
    a = [zeros(1,xnum); zeros(ynum-1,1) a];
...
for i=1:1:ynum
    for j=1:1:xnum
        ...
        if ( (i==1) || (j==1) || (i==2 && j==2) || (i==2 && j==xnum) || (i==ynum && j==2) || (i==ynum && j==xnum) || (i==p0celly && j==p0cellx && pChoice == RELAX_CC))
            %         if ( (i==1) || (j==1) || (i==2 && j==2) || (i==2 && j==xnum) || (i==ynum && j==2) || (i==ynum && j==xnum) || (i==p0celly && j==p0cellx && pChoice == RELAX_CC)...
            %                 || (((i==p0celly && j == p0cellx) || (i == p1celly && j == p1cellx) || (i == p2celly && j == p2cellx) || (i == p3celly && j == p3cellx) ...
            %                 || (i== p4celly && j == p4celly)) && pSourcesD == true) )
            %             %           if ( (i==1) || (j==1) || (i==2 && j==3))
            ...
            if((i==2 && j==2) || (i==ynum && j==2))
                ...
                Lf(inp,inp+ynum*3,-1*kbond); % Coefficient for P(i,j+1)
                R(inp,1)            =   0;          % Right-hand-side part
            end
            ...
            % One cell
            if (i==p0celly && j==p0cellx && pChoice == RELAX_CC) %relax CC at p0cell
                Lf(inp,inp,1*kbond);    % Coefficient for P(i,j)
        ...
        if(i==1 || i==ynum-1 || i==ynum || j==1 || j==xnum)
            ...
            %             %mu(d2vx/dx2+2vx/dy2)=mu*((vx(i,j-1)-2vx(i,j)+vx(i,j+1))/dx^2+(vx(i-1,j)-2vx(i,j)+vx(i+1,j))/dy^2)
            %             L(invx,invx-ynum*3)     =   mu(i,j)/xstp^2;                 % Coefficient for vx(i,j-1)
            %             L(invx,invx-3)          =   mu(i,j)/ystp^2;                 % Coefficient for vx(i-1,j)
            %             L(invx,invx)            =   -2*mu(i,j)/xstp^2-2*mu(i,j)/ystp^2; % Coefficient for vx(i,j)
            %             L(invx,invx+3)          =   mu(i,j)/ystp^2;                 % Coefficient for vx(i+1,j)
            %             L(invx,invx+ynum*3)     =   mu(i,j)/xstp^2;                 % Coefficient for vx(i,j+1)
            %
            %             % -dP/dx=(P(i+1,j)-P(i+1,j+1))/dx
            ...
            %             % Right-hand-side part: (mu+lambda)da/dx = (mu +
            %             % lambda)(a(i+1,j+1)-a(i+1,j))/dx
            %             R(invx,1)               =  (mu(i,j)+lambda(i,j))* (a(i+1,j+1)-a(i+1,j))/xstp;
        end
