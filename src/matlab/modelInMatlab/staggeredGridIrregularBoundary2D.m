% Test of staggered grid with irregular boundary;
...
for j=1:1:yn
    for i=1:1:xn
        ...
        %statements!!)
        bStBndCell1.i = i+1;    bStBndCell1.j = j+1;
        bStBndCell2.i = i;    bStBndCell2.j = j+1;
        ...
        elseif(j==yn-1)   %North wall: 3vx(i,j) - vx(i,j-1) = 2nx
            ...
            M(sub2ind([N N],row*ones(1,2),getPos2D(colSts(1:2)))) = v(1:2);
        elseif(bMask(getPos2D(bStBndCell1))==0 || bMask(getPos2D(bStBndCell2))==0) % skull or outside
            % vx(i,j) = 0;
            M(row,:) = 0;
            colSts(1).i = i;    colSts(1).j = j;    colSts(1).c = cVx;
            M(row,getPos2D(colSts(1))) = dbScale;
        ...
        %statements!!)
        bStBndCell1.i = i+1;    bStBndCell1.j = j+1;
        bStBndCell2.i = i+1;    bStBndCell2.j = j;
        ...
        bStBndCell1.i = i;    bStBndCell1.j = j; % For Dirichlet skull
        bStBndCell2.i = i+1;    bStBndCell2.j = j;
        bStBndCell3 = bStBndCell1;  bStBndCell4 = bStBndCell1;
        bStBndCell5 = bStBndCell1;
        bStBndCell3.i = i-1;    bStBndCell3.j = j;
        bStBndCell4.i = i;      bStBndCell4.j = j+1;
        bStBndCell5.i = i;      bStBndCell5.j = j-1;
        if(bMask(getPos2D(bSt))==1)%(bMask(getPos2D(bSt))<2)
%         if(bMask(getPos2D(bSt1))==1)%(bMask(getPos2D(bSt))<2)
            num = num+1;
            colSts(num).i = i;  colSts(num).j = j;
        ...
        % corners.
        bSt1.i = i;    bSt1.j = j;    % For Dirchlet boundary on the skull
        ...
        elseif(i==xn && (j==2 || j==yn)) %east corners
            M(row,:) = 0;
            M(row,row) = dbScale; %For Dirichlet condition.
        elseif((bMask(getPos2D(bStBndCell1))==0) || ...
               (bMask(getPos2D(bStBndCell2))==0 && bMask(getPos2D(bStBndCell3))==0 && ...
                bMask(getPos2D(bStBndCell4))==0 && bMask(getPos2D(bStBndCell5))==0)...
              ) %Skull or outside
            M(row,:) = 0;
            M(row,row) = dbScale; %For Dirichlet condition.
...
figure,
imagesc(rot90(bMask)), colormap(gray), axis image;
hold on;
quiver(x,y,vxC,vyC), axis image, title('velocity field');
