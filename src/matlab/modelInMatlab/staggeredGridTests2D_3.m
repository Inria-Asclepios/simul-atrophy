...
% This show HOW addition of fictious cells only to the pressure variable
% and not having diff. number of variables for vx and vy still does not
% solve the problem! 
...
% -------------------------------------------------------------------------        
        ...
        %Prioritize those that set directly without interpolation (effects
        %orderings of the elseif statements!!)
        if(i==1)    %West wall: vx = wx
            M(row,:) = 0;
        ...
        elseif(i==xn)   %East wall: vx = ex
            ...
            colSts(1).i = i;    colSts(1).j = j;    colSts(1).c = 0;
            M(row,getPos2D(colSts(1))) = dbScale;
        elseif(j==1)        %South wall: vx(i,j) + vx(i,j+1) = 2sx
            M(row,:) = 0;
            colSts(1).i = i;    colSts(1).j = j;
            v(1) = dbScale;   colSts(1).c = 0;
...
% ------------------------------------------------------------------------        
        ...
        % thus make the rank-deficient matrix. These are fictious pressure
        % nodes which we can set to whatever we like but will not affect
        % other values. Let's set them to 0. (LATER, for now let's also see
        % with neumann condition, what it does!)
        
        if(j==1)    %South wall
            M(row,:) = 0;
%             colSts(1).i = i;    colSts(1).j = j;
%             v(1) = -dbScale;   colSts(1).c = 2;
%             colSts(2).i = i;    colSts(2).j = j+1;  % PROLLY SHOULD NOT LINK IT UP WITH ACTUAL PRESSURE VARIABLES!!
...
%             M(sub2ind([N N],row*ones(1,2),getPos2D(colSts(1:2)))) = v(1:2);
            M(row,row) = dbScale;
        elseif(i==xn && j==yn) %top-right corner
            M(row,:) = 0;
%             colSts(1).i = i;    colSts(1).j = j;
%             v(1) = -dbScale;   colSts(1).c = 2;
